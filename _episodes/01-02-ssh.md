...
the remote machine's command prompt is `master$`
instead of just `$`.
...
For example, this command copies our latest results to the HPC cluster,
printing out its progress as it does so:

~~~
$ scp results.dat userNN@{{ site.workshop_login_host }}:data/results-2018-04.dat
Password: ********
~~~
{: .bash}
...
~~~
$ scp results.dat userNN@{{ site.workshop_login_host }}:
~~~
{: .bash}

would copy `results.dat` into our home directory on `{{ site.workshop_login_host }}`, however, if we did not
...
but now we have merely created a file called `userNN@{{ site.workshop_login_host }}` on our local machine,
as we would have done with `cp`.

~~~
$ cp results.dat userNN@{{ site.workshop_login_host }}
~~~
{: .bash}
...
For example,
this command copies all of our files from 'data' directory on the {{ site.workshop_login_host }} 
server to 'backup' directory on our laptop:

~~~
$ scp -r userNN@{{ site.workshop_login_host }}:data ./backups
Password: ********
~~~
{: .bash}
...
~~~
$ ssh userNN@{{ site.workshop_login_host }} "ls results*"
Password: ********
~~~
{: .bash}
...
~~~
    master$ nano ~/.ssh/authorized_keys`.
~~~
{: .bash}
